...
class documentClassifier:
    ...
    def getFeatureCountsPerCategory(self,currentFeature,currentCategory):
        if currentFeature in self.featureCountsPerCategory and currentCategory in self.featureCountsPerCategory[currentFeature]:
    ...
    def getFeaturePerCategoryProbability(self,currentFeature,currentCategory):
        ...
        return self.getFeatureCountsPerCategory(currentFeature,currentCategory)/self.getCategoryCount(currentCategory)
    ...
    def weightedprob(self,currentFeature,currentCategory,prf,weight=1.0,ap=0.5):
        ...
        # vo site kategorii
        totals=sum([self.getFeatureCountsPerCategory(currentFeature,currentCategory) for currentCategory in self.categories()])
...
if __name__ == "__main__":
    ...
    recenica = input()
    zborovi = cl.getfeatures(recenica).keys()
    kategorii = cl.categories()
    for zbor in zborovi:
        for kategorija in kategorii:
            if kategorija == "science":
                verojatnostNaZbor = round(cl.getFeaturePerCategoryProbability(zbor,'science'), 4)
                verojatnostNaZborTezinska = round(cl.weightedprob(zbor, 'science', cl.getFeaturePerCategoryProbability), 4)
            else:
                verojatnostNaZbor = round(cl.getFeaturePerCategoryProbability(zbor, 'sport'), 4)
                verojatnostNaZborTezinska = round(cl.weightedprob(zbor, 'sport', cl.getFeaturePerCategoryProbability), 4)
            print zbor, kategorija, verojatnostNaZbor, verojatnostNaZborTezinska
    klasa=cl.classifyDocument(recenica)
    verojatnost = math.log(cl.getCategoryProbabilityForDocument(recenica, klasa),2)
    print klasa, round(verojatnost,4)